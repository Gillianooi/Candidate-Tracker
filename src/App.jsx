...
export default function App() {
  ...
  const [detailTab, setDetailTab] = useState("info"); // info | title | dates
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!r?.value) {
          for (const oldKey of ["mob_cand_v4","mob_cand_v3","mob_cand_v2","mob_candidates_v1"]) {
            try {
              const old = await window.storage.get(oldKey);
              if (old?.value) {
                r = old;
                await window.storage.set(STORAGE_KEY, old.value);
                break;
              }
            } catch {}
          }
  ...
  const DATE_TYPES = {
    meeting:      {label:"🤝 Meeting / Consultation",    color:"#7eb3ff"},
    followup:     {label:"📞 Follow-up Meeting",          color:"#7eb3ff"},
    presentation: {label:"🏆 Record Presentation",        color:"#ffd166"},
    event:        {label:"🎉 Event Date",                 color:"#ffd166"},
    submission:   {label:"📋 Document Submission",        color:"#6bffb8"},
    payment:      {label:"💰 Payment Date",               color:"#6bffb8"},
    approval:     {label:"✅ Approval / Confirmation",    color:"#6bffb8"},
  ...
  const addMeetingDate = (id) => {
    ...
    const u = candidates.map(c => {
      if (c.id !== id) return c;
      const dates = [...(c.meetingDates||[]), {date: newMeetingDate, type: newMeetingType, note: newMeetingNote.trim(), added: new Date().toISOString()}];
      return {...c, meetingDates: dates.sort((a,b) => new Date(b.date)-new Date(a.date)), lastUpdated:new Date().toISOString()};
    });
    persist(u); setSelected(u.find(c => c.id === id));
    setNewMeetingDate(""); setNewMeetingNote(""); setNewMeetingType("meeting"); setAddingMeetingDate(false);
  };

  const addAndSyncCalendar = async (c) => {
    if (!newMeetingDate) return;
    // First save the date
    addMeetingDate(c.id);
    // Then create Google Calendar event via Anthropic API
    const dt = DATE_TYPES[newMeetingType]||DATE_TYPES.other;
    const candidateName = [c.name, c.org].filter(Boolean).join(" – ");
    const startTime = new Date(newMeetingDate).toISOString();
    const endTime = new Date(new Date(newMeetingDate).getTime() + 60*60*1000).toISOString(); // +1 hour
    const eventTitle = `${dt.label.split(" ").slice(1).join(" ")}: ${candidateName}`;
    const eventDesc = `ASEAN/Asia Records
Step ${c.step}: ${STEP_NAMES[c.step]||""}
Status: ${c.status}${newMeetingNote?`
Note: ${newMeetingNote}`:""}`;
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages:[{role:"user",content:`Create a Google Calendar event with these details:
Title: "${eventTitle}"
Start: ${startTime}
End: ${endTime}
Description: ${eventDesc}
Use the calendar tool to create this event. Return only "SUCCESS" if created.`}]
        })
      });
      const data = await res.json();
      const text = data.content?.find(b=>b.type==="text")?.text||"";
      if(text.includes("SUCCESS")||text.toLowerCase().includes("created")||text.toLowerCase().includes("success")) {
        alert("✅ Saved & added to Google Calendar!");
      } else {
        alert("✅ Date saved! Calendar sync: " + text.slice(0,100));
      }
    } catch(e) {
      alert("✅ Date saved locally. Calendar sync failed – try again.");
    }
  };
  ...
  const saveRecordTitle = (id, titleData) => {
    const u = candidates.map(c => c.id === id ? {...c, recordTitle: titleData, lastUpdated: new Date().toISOString()} : c);
    persist(u); setSelected(u.find(c => c.id === id));
  };

  const openEdit = (c) => {
    setEditStatus(c.status); setEditStep(c.step); setEditNote(c.notes||"");
    setEditName(c.name||""); setEditOrg(c.org||""); setEditing(true);
  };
      ...
      <div style={BASE}>
        ...
        <div style={{padding:16,paddingBottom:80}}>
          ...
          <div style={BOX()}>
            ...
            <button onClick={()=>{try{navigator.clipboard.writeText(textExport).then(()=>{alert("✅ Copied!")});}catch(e){alert("Please manually copy the text.");}}} style={BTN("#1a2e52","#7eb3ff",{border:"1px solid #2a5298"})}>
  ...
  if(view==="detail"&&selected) {
    ...
    const rt = c.recordTitle||{};

    return (
      <div style={BASE}>
        <div style={HDR({justifyContent:"space-between"})}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button onClick={()=>{setView("list");setEditing(false);setDetailTab("info");}} style={BACK}>←</button>
        ...
        <div style={{padding:16,paddingBottom:80}}>
          ...
          {!editing&&(
            <div style={{display:"flex",gap:6,marginBottom:12}}>
              {[["info","📋 Info"],["title","🏆 Title"],["dates","📅 Dates"]].map(([k,l])=>(
                <button key={k} onClick={()=>setDetailTab(k)} style={{flex:1,padding:"9px 4px",borderRadius:8,cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700,border:"1px solid",background:detailTab===k?"#1e3a6e":"#161922",color:detailTab===k?"#7eb3ff":"#555",borderColor:detailTab===k?"#2a5298":"#252a3a"}}>{l}</button>
              ))}
            </div>
          )}
          ...
          {!editing&&detailTab==="info"&&(
            <>
              ...
              {c.notes
                ?<div style={BOX({background:"#0e1a2e",borderColor:"#1e3a6e"})}><div style={{...LBL,color:"#4a6fa5"}}>📝 Notes</div><div style={{fontSize:14,color:"#9bb8e8",lineHeight:1.6,whiteSpace:"pre-wrap"}}>{c.notes}</div></div>
          ...
          {/* TITLE TAB */}
          {!editing&&detailTab==="title"&&(
            <div>
              <div style={BOX({background:"#1a1a0e",borderColor:"#4a4a10"})}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <div style={{...LBL,color:"#ffd166",marginBottom:0}}>🏆 Official Record Title</div>
                  <button onClick={()=>{setEditingTitle(!editingTitle);setEditTitleDraft(rt.official||"");}} style={{background:"#1e3a6e",border:"none",borderRadius:6,color:"#7eb3ff",fontSize:12,fontWeight:700,padding:"4px 10px",cursor:"pointer",fontFamily:"inherit"}}>
                    {editingTitle?"Cancel":"✏️ Edit"}
                  </button>
                </div>
                {editingTitle?(
                  <>
                    <textarea value={editTitleDraft} onChange={e=>setEditTitleDraft(e.target.value)} rows={3}
                      style={{...INP,resize:"none",marginBottom:8}} placeholder="e.g. Largest Collection of Recipe Books by a Single Author in ASEAN"/>
                    <button onClick={()=>{saveRecordTitle(c.id,{...rt,official:editTitleDraft});setEditingTitle(false);}}
                      style={{width:"100%",padding:10,borderRadius:8,border:"none",background:"#2a5298",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>
                      💾 Save Title
                    </button>
                  </>
                ):(
                  rt.official
                    ?<div style={{fontSize:15,color:"#ffd166",fontWeight:600,lineHeight:1.5}}>{rt.official}</div>
                    :<div style={{fontSize:13,color:"#333",fontStyle:"italic"}}>No official title yet. Tap Edit to add.</div>
                )}
              </div>

              <div style={BOX()}>
                <div style={{...LBL,marginBottom:8}}>📝 Proposed Titles</div>
                {(rt.proposed||[]).map((t,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"8px 0",borderTop:i>0?"1px solid #1e2235":"none"}}>
                    <div style={{flex:1,fontSize:13,color:"#ccc",lineHeight:1.5,paddingRight:8}}>{t}</div>
                    <button onClick={()=>{const p=(rt.proposed||[]).filter((_,j)=>j!==i);saveRecordTitle(c.id,{...rt,proposed:p});}} style={{background:"none",border:"none",color:"#333",cursor:"pointer",fontSize:14}}>✕</button>
                  </div>
                ))}
                <div style={{display:"flex",gap:8,marginTop:8}}>
                  <input id="newTitleInput" placeholder="Add proposed title..." style={{flex:1,background:"#161922",border:"1px solid #252a3a",borderRadius:8,color:"#e0e2ea",fontSize:13,padding:"9px 12px",fontFamily:"inherit",outline:"none"}}/>
                  <button onClick={()=>{const inp=document.getElementById("newTitleInput");if(inp&&inp.value.trim()){saveRecordTitle(c.id,{...rt,proposed:[...(rt.proposed||[]),inp.value.trim()]});inp.value="";}}} style={{padding:"9px 14px",borderRadius:8,border:"none",background:"#2a5298",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>+</button>
                </div>
              </div>

              <div style={BOX()}>
                <div style={LBL}>Record Type</div>
                <div style={{display:"flex",gap:8}}>
                  {[["direct","📌 Direct Recognition"],["attempt","🏁 Record Attempt"]].map(([k,l])=>(
                    <button key={k} onClick={()=>saveRecordTitle(c.id,{...rt,type:k})} style={{flex:1,padding:"9px 6px",borderRadius:8,cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700,border:`1px solid ${rt.type===k?"#2a5298":"#252a3a"}`,background:rt.type===k?"#1e3a6e":"#161922",color:rt.type===k?"#7eb3ff":"#555"}}>{l}</button>
                  ))}
                </div>
              </div>

              <div style={BOX()}>
                <div style={LBL}>Title Notes / Context</div>
                <textarea value={rt.notes||""} onChange={e=>saveRecordTitle(c.id,{...rt,notes:e.target.value})} rows={3}
                  style={{...INP,resize:"none"}} placeholder="Background, benchmarks, key metrics..."/>
              </div>
            </div>
          )}

          {/* DATES TAB */}
          {!editing&&detailTab==="dates"&&(
            <div style={BOX({background:"#0e1a2e",borderColor:"#1e3a6e"})}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{...LBL,color:"#4a6fa5",marginBottom:0}}>📅 Date Log</div>
              ...
              {addingMeetingDate&&(
                <div style={{marginBottom:10,background:"#0c1220",borderRadius:8,padding:10}}>
                  <div style={{fontSize:10,color:"#444",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6}}>Date Type</div>
                  <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:10}}>
                    {Object.entries(DATE_TYPES).map(([k,v])=>(
                      <div key={k} onClick={()=>setNewMeetingType(k)} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"7px 10px",borderRadius:7,background:newMeetingType===k?"rgba(126,179,255,0.1)":"transparent",border:`1px solid ${newMeetingType===k?"#2a5298":"transparent"}`}}>
                        <div style={{width:16,height:16,borderRadius:4,border:`2px solid ${newMeetingType===k?"#7eb3ff":"#333"}`,background:newMeetingType===k?"#7eb3ff":"transparent",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                          {newMeetingType===k&&<span style={{fontSize:10,color:"#0c0e13",fontWeight:900}}>✓</span>}
                        </div>
                        <span style={{fontSize:13,color:newMeetingType===k?"#7eb3ff":"#666"}}>{v.label}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{fontSize:10,color:"#444",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6}}>Date & Time</div>
                  <input type="datetime-local" value={newMeetingDate} onChange={e=>setNewMeetingDate(e.target.value)} style={{...INP,marginBottom:8,colorScheme:"dark"}}/>
                  <div style={{fontSize:10,color:"#444",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6}}>Note (optional)</div>
                  <input value={newMeetingNote} onChange={e=>setNewMeetingNote(e.target.value)} style={{...INP,marginBottom:10}} placeholder="e.g. Discussed titles, Paid deposit..."/>
                  <button onClick={()=>addMeetingDate(c.id)} disabled={!newMeetingDate} style={{...BTN("#2a5298"),marginBottom:8,padding:10,fontSize:13,opacity:newMeetingDate?1:0.4}}>💾 Save</button>
                  {newMeetingDate&&<button onClick={()=>addAndSyncCalendar(c)} style={{...BTN("#1a2e52","#7eb3ff",{border:"1px solid #2a5298",marginBottom:0,padding:10,fontSize:13})}}>📅 Save + Export to Calendar (.ics)</button>}
                </div>
              )}
              {(c.meetingDates||[]).length===0&&!addingMeetingDate&&<div style={{fontSize:12,color:"#333"}}>No dates recorded yet.</div>}
              {(c.meetingDates||[]).map((m,i)=>{
                const dt=DATE_TYPES[m.type]||DATE_TYPES.other;
                return(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"8px 0",borderTop:i>0?"1px solid #1a2235":"none"}}>
                    <div style={{flex:1}}>
                      <div style={{fontSize:11,color:dt.color,fontWeight:700,marginBottom:2}}>{dt.label}</div>
                      <div style={{fontSize:13,color:"#7eb3ff",fontWeight:600}}>
                        {new Date(m.date).toLocaleDateString("en-MY",{weekday:"short",day:"numeric",month:"short",year:"numeric"})}
                        {m.date.includes("T")&&m.date.split("T")[1]!=="00:00"?" · "+new Date(m.date).toLocaleTimeString("en-MY",{hour:"2-digit",minute:"2-digit"}):""}
                      </div>
                      {m.note&&<div style={{fontSize:11,color:"#555",marginTop:2}}>{m.note}</div>}
                    </div>
                    <button onClick={()=>deleteMeetingDate(c.id,i)} style={{background:"none",border:"none",color:"#333",cursor:"pointer",fontSize:14,padding:"0 0 0 8px"}}>✕</button>
                  </div>
                );
              })}
            </div>
          ...
          <button style={BTN("#1e2235","#888")} onClick={()=>{setView("list");setEditing(false);}}>← Back</button>
        </div>
      </div>
    );
  }
    ...
    <div style={BASE}>
      ...
      <div style={{padding:"4px 16px 80px"}}>
        ...
        {filtered.map(c=>{
          ...
          return(
            <div key={c.id} onClick={()=>{setSelected(c);setEditing(false);setAddingMeetingDate(false);setDetailTab("info");setView("detail");}}
              ...
              {lastMeeting&&<div style={{fontSize:11,color:"#4a6fa5",marginBottom:4}}>📅 {new Date(lastMeeting.date).toLocaleDateString("en-MY",{day:"numeric",month:"short",year:"numeric"})} · {lastMeeting.type==="presentation"?"🏆 Presentation":lastMeeting.type==="event"?"🎉 Event":lastMeeting.type==="payment"?"💰 Payment":lastMeeting.note||"Meeting"}</div>}
              {c.recordTitle?.official&&<div style={{fontSize:11,color:"#ffd166",marginBottom:4}}>🏆 {c.recordTitle.official.length>50?c.recordTitle.official.slice(0,50)+"...":c.recordTitle.official}</div>}
